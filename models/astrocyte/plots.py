...
def plot_astro_dynamics(axe_ip3, events, start=0.0, xlims=(None, None), ylims_ip3=(None, None), ylims_calcium=(None, None)):
    """Plot astrocytic dynamics."""
    # Prepare astrocyte data
...
def plot_neuro_sic(axe, events, start=0.0, xlims=(None, None), ylims=(None, None)):
    """Plot astrocytic SIC input per neuron."""
    # Prepare neuron data
    mask = events["times"] > start
...
def plot_raster(axe, events, xlims=(None, None), ylims=(None, None), n_neurons=100):
    """Create a raster plot of neuronal firing."""
    # Prepare neuron firing data
    ts = events["times"]
...
def plot_hist(axe, events, n_neurons, binwidth=20, xlims=(None, None), ylims=(None, None)):
    """Create a histogram of neuronal firing rate."""
    # Prepare neuronal firing data
    ts = events["times"]
    neurons = events["senders"]
    # Create histogram
...
def make_panels_benchmark_model(
    ...
    """Plot simulation data and save them in figure panel files."""
    # Sreate save path
    os.system(f"mkdir -p {save_path}")
    # Set xlims for all panels
    xlims = (0, 11000.0)
    # Set different ylims for different panels
    ylims_hist_low, ylims_hist_high = (0.0, 25.0), (0.0, 150.0)
    ylims_ip3, ylims_calcium = (-0.01, 0.91), (-0.02, 2.02)
    ylims_sic = (-5.0, 155.0)
    # Set position of axes for all cases
    pos = [0.2, 0.2, 0.6, 0.7]
...
if __name__ == "__main__":
    ...
    # Create plots (panels in figures)
